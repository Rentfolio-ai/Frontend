...
export const VisionLandingPage: React.FC<VisionLandingPageProps> = ({
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0C] text-white font-sans antialiased selection:bg-violet-500/20 overflow-x-hidden">
      ...
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${navScrolled ? 'bg-[#0A0A0C]/70 backdrop-blur-2xl' : ''}`}>
        <div className="max-w-[1080px] mx-auto px-6 h-14 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[14px] font-display font-semibold text-white/90 tracking-tight">Vasthu Vision</span>
          </div>
          <div className="hidden md:flex items-center gap-8 text-[13px] text-white/30">
            <button onClick={() => scrollTo('features')} className="hover:text-white/70 transition-colors duration-300">Features</button>
            <button onClick={() => scrollTo('pricing')} className="hover:text-white/70 transition-colors duration-300">Pricing</button>
            <a href="/" className="hover:text-white/70 transition-colors duration-300">Vasthu AI</a>
          </div>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <div className="flex flex-col h-full px-6 pt-6">
              <div className="flex items-center justify-between mb-20">
                <span className="text-[14px] font-display font-semibold text-white/90">Vasthu Vision</span>
              ...
              <div className="space-y-8">
                ...
                <a href="/" className="text-[32px] font-display font-medium text-white/25 hover:text-white/70 block w-full text-left tracking-tight transition-colors duration-300">
                  Vasthu AI
                </a>
              </div>
              <div className="mt-auto pb-12 space-y-3">
                <button onClick={() => { setMobileMenuOpen(false); onNavigateToSignUp(); }} className="w-full py-3 rounded-full bg-violet-500 text-white font-medium text-[14px]">Get started</button>
      ...
      <section className="pt-40 md:pt-48 px-6">
        ...
        >
          ...
          <p className="text-[16px] md:text-[17px] text-white/25 leading-[1.75] mb-10 max-w-[440px] mx-auto">
            Point your camera at any room. Instant damage detection, condition assessment, and renovation cost estimates powered by AI.
          </p>
          {recentUser ? (
            <button onClick={handleContinue} className={`inline-flex items-center gap-2 px-6 py-2.5 rounded-full bg-violet-500 text-white font-medium text-[13px] hover:bg-violet-400 transition-colors duration-300 ${shimmerBtn}`}>
              Continue as {recentUser.name.split(' ')[0]} <ArrowRightIcon className="w-3.5 h-3.5" />
            </button>
          ) : (
            <button onClick={onNavigateToSignUp} className={`inline-flex items-center gap-2 px-6 py-2.5 rounded-full bg-violet-500 text-white font-medium text-[13px] hover:bg-violet-400 transition-colors duration-300 ${shimmerBtn}`}>
        ...
        <div className="max-w-[640px] mx-auto mt-20 overflow-hidden">
          <motion.div
            ...
            style={{ y: heroY }}
          >
            <div className="rounded-[20px] overflow-hidden aspect-[4/3] relative bg-[#111] shadow-[0_8px_80px_rgba(139,92,246,0.08)] border border-white/[0.03]">
      ...
      <section id="features" className="scroll-mt-20">
        ...
        <div className="py-24 md:py-32 px-6 bg-[#09090B]">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.3 }} variants={reveal}
            className="max-w-[1020px] mx-auto grid md:grid-cols-2 gap-14 md:gap-20 items-center"
          >
      ...
      {/* ─── Pricing ─────────────────────────────────────────────── */}
      <section id="pricing" className="py-28 md:py-36 px-6 scroll-mt-20 bg-[#09090B]">
          ...
          <div className="grid md:grid-cols-2 rounded-[16px] overflow-hidden border border-white/[0.03] shadow-[0_2px_30px_rgba(0,0,0,0.25)]">
            ...
            <div className="p-8 md:p-10 flex flex-col h-full border-b md:border-b-0 md:border-r border-white/[0.03] bg-[#0A0A0C]">
              <div className="mb-8">
                ...
                <p className="text-white/15 text-[13px] mt-3">Try Vision on a few rooms.</p>
              </div>
            ...
            <div className="p-8 md:p-10 flex flex-col h-full bg-[#0C0C0E]">
              <div className="mb-8">
                ...
                <p className="text-white/15 text-[13px] mt-3">Unlimited scans. Full Vasthu suite.</p>
              </div>
              <ul className="space-y-3 mb-10 flex-1">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 mb-14">
            <div className="col-span-2 md:col-span-1">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-[12px] font-display font-medium text-white/25">Vasthu Vision</span>
              </div>
              <p className="text-white/12 text-[11px] leading-relaxed max-w-[180px]">AI-powered property damage detection.</p>
            </div>
            <div>
              ...
              <div className="space-y-2">
                ...
                <a href="/" className="block text-[12px] text-white/20 hover:text-white/45 transition-colors duration-300">Vasthu AI</a>
              </div>
            </div>
            <div>
              <p className="text-[10px] text-white/15 uppercase tracking-[0.1em] mb-3 font-medium">Company</p>
              <div className="space-y-2">
                <button onClick={() => scrollTo('features')} className="block text-[12px] text-white/20 hover:text-white/45 transition-colors duration-300">Features</button>
