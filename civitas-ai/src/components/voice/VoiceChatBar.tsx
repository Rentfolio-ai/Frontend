    ...
    <div className="space-y-2">
      ...
      <AnimatePresence>
        {session.camera.isActive && (
          ...
          >
            <div className="relative w-[120px] h-[120px] rounded-2xl overflow-hidden border border-white/[0.15] shadow-2xl shadow-black/40">
              <video
              ...
              <div className="absolute top-2 left-2 flex items-center gap-1 px-1.5 py-0.5 rounded-md bg-black/40 backdrop-blur-sm">
                ...
                <span className="text-[9px] font-bold text-white/80 tracking-wider">LIVE</span>
              </div>
            </div>
          </motion.div>
        )}
      ...
      <AnimatePresence>
        ...
        {session.sessionExpired && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0 }}
            className="flex items-center justify-center gap-3 px-3 py-2 rounded-lg bg-white/[0.06] border border-white/[0.10]"
          >
            <span className="text-white/60 text-xs">Session ended</span>
            <button
              onClick={() => session.reconnect()}
              className="px-3 py-1 rounded-full bg-white/[0.08] hover:bg-white/[0.14] text-white/70 hover:text-white text-xs font-medium flex items-center gap-1.5 transition-all"
            >
        ...
        {session.error && !session.sessionExpired && (
          ...
          >
            ...
            {isError && (
              <button
                onClick={() => session.reconnect()}
                className="px-3 py-1 rounded-full bg-white/[0.06] hover:bg-white/[0.10] text-white/50 hover:text-white text-xs font-medium flex items-center gap-1.5 transition-all shrink-0"
              >
                <RotateCcw className="w-3 h-3" />
                Retry
              </button>
      ...
      {/* Main bar */}
      <div className="rounded-2xl backdrop-blur-xl bg-white/[0.08] border border-white/[0.12] hover:border-white/[0.18] transition-all duration-200 shadow-2xl shadow-black/20">
        {/* Row 1: Status strip */}
        <div className="flex items-center gap-3 px-4 py-2 border-b border-white/[0.06]">
          ...
          {persona && (
            ...
            >
              ...
              <span className="text-white/50 text-xs font-medium">{persona.name}</span>
            </button>
          )}

          {/* Status */}
          <span className="text-white/30 text-[11px]" role="status" aria-live="polite">{statusText}</span>
          ...
          <span
            className={`text-[11px] font-mono tabular-nums ${
              timerWarning ? 'text-amber-400' : 'text-white/20'
            }`}
          >
            {timerText}
          </span>
        ...
        <div className="flex items-center gap-2 px-4 py-2.5">
          ...
          {text.trim() && (
            <button
              ...
              className="p-1.5 rounded-lg bg-white/[0.08] hover:bg-white/[0.14] text-white/70 hover:text-white transition-all"
            >
          ...
          <button
            onClick={onEnd}
            className="flex items-center gap-1.5 pl-2.5 pr-3 py-1.5 rounded-full bg-blue-500 hover:bg-blue-600 text-white text-sm font-medium transition-all"
