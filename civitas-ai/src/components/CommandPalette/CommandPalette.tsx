        ...
        <AnimatePresence>
            <div className="fixed inset-0 z-[100] flex items-start justify-center p-4 pt-[20vh]">
                ...
                <motion.div
                    ...
                    className="relative w-full max-w-2xl bg-popover border border-black/8 rounded-2xl shadow-2xl overflow-hidden"
                >
                    {/* Search Input */}
                    <div className="flex items-center gap-3 px-4 py-3 border-b border-black/8">
                        ...
                        />
                        <kbd className="hidden sm:flex items-center gap-1 px-2 py-1 rounded-md bg-black/5 text-muted-foreground/70 text-xs font-mono">
                            <span>ESC</span>
                        </kbd>
                    </div>
                    ...
                    <div ref={listRef} className="max-h-[400px] overflow-y-auto custom-scrollbar">
                        {groupedCommands.length === 0 ? (
                            <div className="py-12 text-center">
                                <p className="text-muted-foreground/70 text-sm">No commands found</p>
                            </div>
                        ) : (
                            groupedCommands.map((group, groupIdx) => (
                                <div key={group.category} className={groupIdx > 0 ? 'mt-1' : ''}>
                                    {/* Category Label */}
                                    <div className="px-4 py-2 text-xs font-semibold text-muted-foreground/50 uppercase tracking-wider bg-black/[0.02]">
                                        {group.label}
                                    </div>
                                            ...
                                            >
                                                ...
                                                {command.icon && (
                                                    <command.icon className={cn(
                                                        ...
                                                        isSelected ? 'text-blue-400' : 'text-muted-foreground/70'
                                                    )} />
                                                )}
                                                ...
                                                {/* Shortcut */}
                                                {command.shortcut && (
                                                    <kbd className="hidden sm:flex items-center px-2 py-1 rounded-md bg-black/5 text-muted-foreground/70 text-xs font-mono">
                                                        {command.shortcut}
                                                    </kbd>
                                                )}
                                            </button>
                    ...
                    {/* Footer */}
                    <div className="flex items-center justify-between px-4 py-2 border-t border-black/8 bg-black/[0.02]">
                        <div className="flex items-center gap-4 text-xs text-muted-foreground/70">
                            <span className="flex items-center gap-1">
                                <kbd className="px-1.5 py-0.5 rounded bg-black/5">↑</kbd>
                                <kbd className="px-1.5 py-0.5 rounded bg-black/5">↓</kbd>
