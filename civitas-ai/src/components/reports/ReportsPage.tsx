...
const REC_CONFIG: Record<string, { label: string; dotClass: string; textClass: string }> = {
  ...
  pass: { label: 'Pass', dotClass: 'bg-white/25', textClass: 'text-muted-foreground/60' },
  ...
  monitor: { label: 'Monitor', dotClass: 'bg-white/30', textClass: 'text-muted-foreground/70' },
};

const getReportConfig = (type: string): ReportTypeConfig =>
  REPORT_CONFIG[type] || { label: type, shortLabel: type, color: '#555', textClass: 'text-muted-foreground', bgClass: 'bg-black/5' };

const getRecConfig = (rec: string) =>
  REC_CONFIG[rec.toLowerCase()] || { label: rec, dotClass: 'bg-white/40', textClass: 'text-muted-foreground' };
...
const KPIStrip: React.FC<{ reports: ReportSummary[] }> = ({ reports }) => {
  ...
  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-1 mb-4">
      {kpis.map((kpi) => (
        <div key={kpi.label} className="rounded-lg bg-black/[0.02] border border-black/[0.04] px-3 py-2.5">
          <div className="flex items-center justify-between mb-1.5">
            <p className="text-[10px] text-muted-foreground/50 font-medium">{kpi.label}</p>
            <kpi.icon className="w-3 h-3 text-muted-foreground/40" />
          </div>
          <p className="text-[17px] font-mono font-bold text-foreground tracking-tight leading-none">{kpi.value}</p>
        </div>
      ))}
    </div>
  );
...
const EmptyState: React.FC<{ onAnalyze: () => void }> = ({ onAnalyze }) => (
  <div className="flex flex-col items-center justify-center py-20 text-center">
    ...
    <h2 className="text-[14px] font-medium text-foreground/70 mb-1">No reports yet</h2>
    <p className="text-[12px] text-muted-foreground/50 mb-5">
      Analyze a property in chat to generate your first report.
    </p>
    <button
      onClick={onAnalyze}
      className="flex items-center gap-2 px-4 py-2 rounded-lg bg-black/[0.05] hover:bg-black/[0.06] text-[12px] font-medium text-muted-foreground hover:text-foreground/80"
    >
  ...
  <div className="space-y-4">
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-1">
      {Array.from({ length: 4 }).map((_, i) => (
        <div key={i} className="h-[60px] rounded-lg bg-black/[0.02] border border-black/[0.04] animate-pulse" />
      ))}
    </div>
    <div className="rounded-lg bg-black/[0.02] border border-black/[0.04] overflow-hidden">
      {Array.from({ length: 6 }).map((_, i) => (
        <div key={i} className="h-10 border-b border-black/[0.05] animate-pulse" />
      ))}
    </div>
  </div>
);
...
const ReportRow: React.FC<{
  ...
  return (
    <tr onClick={onView} className="group hover:bg-black/[0.02] cursor-pointer border-b border-black/[0.04] last:border-b-0">
      <td className="px-3 py-2.5">
        <span className="px-1.5 py-0.5 rounded text-[9px] font-semibold bg-black/[0.04] text-muted-foreground">
          {cfg.shortLabel}
        </span>
      </td>
      <td className="px-3 py-2.5">
        <p className="text-[12px] text-muted-foreground/80 group-hover:text-foreground truncate max-w-[300px]" title={report.property_address}>
      ...
      <td className="px-3 py-2.5">
        <div className="flex items-center gap-2.5 text-[10px] font-mono text-muted-foreground/50">
      ...
      <td className="px-3 py-2.5">
        <span className="text-[10px] font-mono text-muted-foreground/40">{fmtDate(report.created_at)}</span>
      </td>
      <td className="px-3 py-2.5 text-right">
        <div className="flex items-center justify-end gap-0.5 opacity-0 group-hover:opacity-100">
          <button onClick={onOpenTab} className="p-1 rounded hover:bg-black/[0.06] text-muted-foreground/50 hover:text-muted-foreground" title="Open in new tab">
            <ExternalLink className="w-3 h-3" />
          </button>
          <button onClick={onDelete} className="p-1 rounded hover:bg-red-500/10 text-muted-foreground/50 hover:text-red-400" title="Delete">
...
const ReportViewerModal: React.FC<{
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-6xl h-[90vh] rounded-xl overflow-hidden flex flex-col bg-card border border-black/[0.08] shadow-2xl shadow-black/10"
        ...
        <div className="flex items-center justify-between px-5 py-3 border-b border-black/[0.06] flex-shrink-0">
          <div className="min-w-0">
            <div className="flex items-center gap-2 mb-0.5">
              ...
              <h3 className="text-[13px] font-semibold text-foreground">{cfg ? cfg.label : 'Report'}</h3>
            </div>
            {reportAddress && <p className="text-[11px] text-muted-foreground/60 truncate ml-[26px]">{reportAddress}</p>}
          </div>
          <div className="flex items-center gap-1 flex-shrink-0">
            <button onClick={() => window.open(htmlUrl, '_blank')} className="p-1.5 rounded hover:bg-black/[0.05] text-muted-foreground/50 hover:text-muted-foreground" title="Open in new tab">
              <ExternalLink className="w-3.5 h-3.5" />
            </button>
            <button onClick={() => iframeRef.current?.contentWindow?.print()} className="p-1.5 rounded hover:bg-black/[0.05] text-muted-foreground/50 hover:text-muted-foreground" title="Print / Save PDF">
              <Printer className="w-3.5 h-3.5" />
            </button>
            <div className="w-px h-4 mx-1 bg-black/[0.05]" />
            <button onClick={onClose} className="p-1.5 rounded hover:bg-black/[0.05] text-muted-foreground/50 hover:text-muted-foreground">
...
export const ReportsPage: React.FC<{ onNavigateToChat?: () => void; onBack?: () => void }> = ({ onNavigateToChat, onBack }) => {
  ...
  return (
    <motion.div
      ...
      className="h-full flex flex-col overflow-hidden bg-background relative"
    >
      <div className="px-6 pt-6 flex-shrink-0 relative z-10">
          ...
          <div className="flex items-center justify-between gap-3">
            <div className="flex items-center gap-2 flex-shrink-0">
              {onBack && (
                <button
                  onClick={onBack}
                  className="w-8 h-8 rounded-lg hover:bg-black/[0.03] border border-transparent hover:border-black/[0.08] flex items-center justify-center transition-all group -ml-1"
                  title="Back to Home"
                >
              ...
              <div className="flex items-center gap-2 flex-1 max-w-[320px]">
                <div className="relative flex-1">
                  ...
                  <input
                    ...
                    className="w-full pl-8 pr-3 py-1.5 rounded-lg bg-black/[0.02] border border-black/[0.06]
                               text-[12px] text-foreground/80 placeholder:text-muted-foreground/40
                               focus:outline-none focus:border-[#C08B5C]/30 focus:ring-1 focus:ring-[#C08B5C]/20
                               transition-all duration-150"
                    data-wand-id="reports-search"
                    data-wand-type="input"
                    data-wand-label="Reports search"
                  />
                </div>
                <button
                  onClick={() => loadReports(true)}
                  disabled={isRefreshing}
                  className="w-7 h-7 rounded-lg bg-black/[0.02] border border-black/[0.06] flex items-center justify-center
                             text-muted-foreground/50 hover:text-[#D4A27F]/70 hover:border-[#C08B5C]/20 transition-all
        ...
        {hasReports && (
          <div className="flex items-center gap-0 mt-4 border-b border-black/[0.06]">
            {TABS.map(tab => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    isActive ? "text-foreground" : "text-muted-foreground/60 hover:text-foreground/55"
                  )}
                >
      ...
      <div className="flex-1 overflow-y-auto px-6 pt-4 pb-6 relative z-10" style={{ scrollbarWidth: 'thin', scrollbarColor: 'rgba(0,0,0,0.06) transparent' }}>
        {isLoading ? (
          <LoadingSkeleton />
        ) : !hasReports ? (
          ...
          <>
            <span className="text-[12px] text-muted-foreground/50 font-medium">Key Metrics</span>
            <div className="mt-2">
            ...
            {filteredReports.length === 0 ? (
              ...
              <div className="rounded-lg overflow-hidden bg-black/[0.02] border border-black/[0.04]">
                    ...
                    <tr className="border-b border-black/[0.05]">
                      {[
                        ...
                        <th
                          key={i}
                          className={cn(
                            "px-3 py-2.5 text-[10px] uppercase font-semibold text-muted-foreground/50 tracking-wider select-none",
                            col.width,
                            col.field && "cursor-pointer hover:text-muted-foreground",
                            i === 5 && "text-right"
                          )}
