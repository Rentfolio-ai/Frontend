...
const SpotlightCard: React.FC<{ children: React.ReactNode; className?: string }> = ({ children, className = '' }) => (
  <div className={`relative rounded-xl bg-[#1C1C21] border border-white/[0.06] overflow-hidden ${className}`}
    ...
    return (
      <SpotlightCard>
        <div className="p-6">
          ...
          <div className="h-9 w-40 rounded bg-white/[0.04] mb-6" />
      ...
      <div className="p-6">
        ...
        <div className="mb-6">
          <div className="flex items-center gap-1.5 mb-2">
            ...
            <span className="text-[10px] uppercase tracking-widest text-white/25">Total Value</span>
          </div>
          <span className="text-[36px] font-bold font-mono text-white/95 tracking-tight leading-none">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-x-0 gap-y-4">
          {secondaryKpis.map((kpi, i) => (
            <div
              key={kpi.label}
              className={`px-3 py-1 ${i < secondaryKpis.length - 1 ? 'md:border-r md:border-white/[0.04]' : ''}`}
            >
              <div className="flex items-center gap-1.5 mb-1.5">
                <kpi.icon className="w-3 h-3 text-white/20" />
                <span className="text-[10px] uppercase tracking-widest text-white/25">{kpi.label}</span>
              </div>
