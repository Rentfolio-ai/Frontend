    ...
    <div className="flex items-center gap-1.5 flex-wrap">
      {items.map((item) => (
        ...
        >
          ...
          <span className="text-[12px] text-muted-foreground/70 group-hover:text-muted-foreground">
            {item.label}
          </span>
          {item.onClick && (
            <ArrowRight className="w-3 h-3 text-muted-foreground/30 group-hover:text-[#C08B5C] transition-colors duration-100" />
          )}
        </button>
      ))}
    </div>
