...
export const SimplePropertyCard: React.FC<SimplePropertyCardProps> = ({
  ...
  return (
    <div className="group relative bg-gradient-to-br from-white via-[#FCF8F4] to-white rounded-3xl p-5 shadow-xl shadow-black/10 hover:shadow-2xl hover:shadow-purple-500/10 transition-all duration-300 transform hover:scale-[1.02] border border-border overflow-hidden w-[320px] min-w-[320px]">
      ...
      <div className="relative z-10">
        ...
        <div className="flex justify-between items-start mb-4">
          ...
          {/* AI Score Badge */}
          {property.ai_score && (
            <div className="ml-auto bg-gradient-to-r from-purple-600 to-blue-600 text-foreground text-xs font-bold px-3 py-1.5 rounded-lg shadow-lg">
        ...
        <div className="grid grid-cols-3 gap-2 mb-3 pb-3 border-b border-border/50">
          {property.beds && (
            <div className="text-center">
              <div className="text-xs text-muted-foreground/70 mb-0.5">Beds</div>
              <div className="text-foreground font-semibold">{property.beds}</div>
            </div>
          )}
          {property.baths && (
            <div className="text-center">
              <div className="text-xs text-muted-foreground/70 mb-0.5">Baths</div>
              <div className="text-foreground font-semibold">{property.baths}</div>
            </div>
          )}
          {property.sqft && (
            <div className="text-center">
              <div className="text-xs text-muted-foreground/70 mb-0.5">SqFt</div>
              <div className="text-foreground font-semibold text-sm">{property.sqft.toLocaleString()}</div>
            </div>
          )}
        </div>

        {/* Additional Amenities - Compact */}
        <div className="grid grid-cols-2 gap-1.5 mb-3 pb-3 border-b border-border/50 text-xs">
          {property.year_built && (
        ...
        <div className="space-y-1.5 mb-3">
          <div className="flex justify-between items-center bg-muted/30 rounded-lg px-2.5 py-1.5">
            <span className="text-muted-foreground text-xs">Est. Rent</span>
        ...
        <button
          ...
          className="w-full py-1.5 bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-500 hover:to-blue-500 text-foreground text-xs font-medium rounded-lg transition-all duration-200 border border-purple-500/30 shadow-lg shadow-purple-500/25 hover:shadow-purple-500/40"
        >
          Analyze Deal →
        </button>
      </div>
