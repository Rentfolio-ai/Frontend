...
export const InteractiveFinancialCard: React.FC<InteractiveFinancialCardProps> = ({
    ...
    return (
        <div className={cn(
            ...
            <div className={cn(
                "bg-[#0f0f11] overflow-hidden",
                compact ? "border-t border-white/[0.06]" : "border border-white/[0.06] rounded-xl shadow-xl shadow-black/80"
            )}>
                ...
                {!compact && (
                    <div className="px-5 py-4 border-b border-white/[0.06] flex items-center justify-between bg-white/[0.02]">
                        <div className="flex items-center gap-2.5">
                            ...
                            <div>
                                <h3 className="text-sm font-semibold text-white/90 tracking-tight">Financial Analysis</h3>
                        ...
                        {params.price > 0 && (
                            <div className="px-2.5 py-1 rounded-md bg-white/[0.04] border border-white/[0.04] text-[11px] font-mono text-white/60">
                ...
                <div className="grid grid-cols-2 gap-px bg-white/[0.06]">
                    {/* Cash Flow */}
                    <div className={cn("bg-[#0f0f11] hover:bg-white/[0.01] transition-colors relative group", compact ? "p-3" : "p-4")}>
                        <div className="text-[9px] uppercase tracking-wider text-white/30 font-semibold mb-1">Monthly Cash Flow</div>
                    ...
                    {/* Cash on Cash */}
                    <div className={cn("bg-[#0f0f11] hover:bg-white/[0.01] transition-colors", compact ? "p-3" : "p-4")}>
                        <div className="text-[9px] uppercase tracking-wider text-white/30 font-semibold mb-1">Cash on Cash</div>
                        <div className={cn("font-bold text-white/90 tracking-tight", compact ? "text-lg" : "text-xl")}>
                    ...
                    {/* Cap Rate */}
                    <div className={cn("bg-[#0f0f11] hover:bg-white/[0.01] transition-colors", compact ? "p-3" : "p-4")}>
                        <div className="text-[9px] uppercase tracking-wider text-white/30 font-semibold mb-1">Cap Rate</div>
                    ...
                    {/* Total Cash Needed */}
                    <div className={cn("bg-[#0f0f11] hover:bg-white/[0.01] transition-colors", compact ? "p-3" : "p-4")}>
                        <div className="text-[9px] uppercase tracking-wider text-white/30 font-semibold mb-1">Total Cash Needed</div>
                ...
                <button
                    ...
                    className="w-full flex items-center justify-between px-5 py-3 text-[11px] font-medium text-white/40 hover:text-white/60 hover:bg-white/[0.02] transition-colors border-t border-white/[0.06]"
                >
                ...
                <AnimatePresence>
                    {isExpanded && (
                        <motion.div
                            ...
                            className="bg-black/20 border-t border-white/[0.06] overflow-hidden"
                        >
                            <div className={cn("space-y-4", compact ? "p-4" : "p-5")}>
                                {/* Purchase Price */}
                                <div className="space-y-1.5">
                                    <label className="text-[10px] uppercase tracking-wide text-white/30 font-semibold">Purchase Price</label>
                                    <div className="relative group">
                                        <DollarSign className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-white/20 group-focus-within:text-indigo-400 transition-colors" />
                                        <input
                                            type="number"
                                            value={params.price}
                                            onChange={(e) => handleParamChange('price', parseFloat(e.target.value) || 0)}
                                            className="w-full bg-white/[0.03] border border-white/[0.08] rounded-lg py-1.5 pl-9 pr-3 text-xs text-white/90 focus:outline-none focus:border-indigo-500/50 focus:bg-white/[0.05] transition-all font-mono"
                                        />
                                    </div>
                                </div>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    {/* Down Payment */}
                                    <div className="space-y-1.5">
                                        <label className="text-[10px] uppercase tracking-wide text-white/30 font-semibold">Down Payment %</label>
                                        <div className="relative group">
                                            <Percent className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-white/20 group-focus-within:text-indigo-400 transition-colors" />
                                            <input
                                                type="number"
                                                step="0.01"
                                    ...
                                    {/* Interest Rate */}
                                    <div className="space-y-1.5">
                                        <label className="text-[10px] uppercase tracking-wide text-white/30 font-semibold">Interest Rate %</label>
                                        <div className="relative group">
                                            <Percent className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-white/20 group-focus-within:text-indigo-400 transition-colors" />
                                            <input
                                                type="number"
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    {/* Renovation Budget */}
                                    <div className="space-y-1.5">
                                        <label className="text-[10px] uppercase tracking-wide text-white/30 font-semibold">Renovation</label>
                                        <div className="relative group">
                                            <DollarSign className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-white/20 group-focus-within:text-indigo-400 transition-colors" />
                                            <input
                                                type="number"
                                    ...
                                    {/* Estimated Rent */}
                                    <div className="space-y-1.5">
                                        <label className="text-[10px] uppercase tracking-wide text-white/30 font-semibold">Est. Rent</label>
                                        <div className="relative group">
                                            <DollarSign className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-white/20 group-focus-within:text-indigo-400 transition-colors" />
                                            <input
                                                type="number"
