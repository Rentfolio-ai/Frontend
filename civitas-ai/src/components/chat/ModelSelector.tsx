...
function getIconForModel(model: ModelInfo): React.ReactNode {
  if (model.id === 'auto') return <Sparkles className="w-3 h-3" />;
...
export const ModelSelector: React.FC<ModelSelectorProps> = ({
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      >
        <span className="text-white/70 [&_svg]:w-3 [&_svg]:h-3">{triggerIcon}</span>
      ...
      <AnimatePresence>
        {open && (
          ...
          >
            <div className="p-1.5 max-h-[320px] overflow-y-auto overscroll-contain scrollbar-thin scrollbar-thumb-white/10 scrollbar-track-transparent">
              {autoModel && (
                <>
                  <ModelRow
                    model={autoModel}
                  ...
                  />
                  <div className="h-px bg-white/[0.06] mx-1.5 my-0.5" />
                </>
              )}
...
function ModelRow({
  ...
  return (
    <button
      type="button"
      disabled={isLocked}
      className={`w-full flex items-center gap-2 px-2 py-[5px] rounded-md text-left transition-colors ${
        isSelected
    ...
    >
      <span className={`shrink-0 ${isSelected ? 'text-white/70' : isLocked ? 'text-white/20' : 'text-white/40'}`}>
        {icon}
      </span>
      <span className={`text-[11px] font-medium truncate flex-1 ${isSelected ? 'text-white' : ''}`}>
        {model.name}
      </span>
      {isLocked && <Lock className="w-2.5 h-2.5 text-white/20 shrink-0" />}
      {isSelected && (
        <div className="w-1.5 h-1.5 rounded-full bg-[#C08B5C] shrink-0" />
      )}
    </button>
  );
}
