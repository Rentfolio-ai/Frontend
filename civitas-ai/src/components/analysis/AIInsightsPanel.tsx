...
export const AIInsightsPanel: React.FC<AIInsightsPanelProps> = ({ insights, className }) => {
    ...
    return (
        <div className={cn("space-y-6", className)}>
            ...
            {/* 1. Property Summary */}
            <div className="bg-slate-50 dark:bg-card/50 rounded-xl p-5 border border-slate-200 dark:border-slate-800">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="bg-white dark:bg-card rounded-xl p-5 border border-slate-200 dark:border-slate-800 shadow-sm">
                        ...
                        <span className={cn(
                            ...
                            insights.strVsLtr.advantage === 'STR' ? "bg-indigo-100 text-indigo-700 dark:bg-indigo-500/20 dark:text-indigo-300" :
                                insights.strVsLtr.advantage === 'LTR' ? "bg-cyan-100 text-cyan-700 dark:bg-cyan-500/20 dark:text-cyan-300" :
                                    "bg-slate-100 text-slate-700 dark:bg-muted dark:text-foreground/70"
                        )}>
                ...
                <div className="bg-white dark:bg-card rounded-xl p-5 border border-slate-200 dark:border-slate-800 shadow-sm">
                    <div className="flex justify-between items-start mb-3">
                        ...
                        <div className="flex flex-col items-end">
                            ...
                            <span className="text-[10px] text-muted-foreground uppercase tracking-wider">Demand Score</span>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2 text-xs font-medium text-muted-foreground/70">
                        ...
                        <span className={cn(
                            ...
                            insights.neighborhoodDemand.trend === 'Up' ? "text-emerald-500" :
                                insights.neighborhoodDemand.trend === 'Down' ? "text-rose-500" : "text-muted-foreground/70"
                        )}>
                ...
                <div className="bg-rose-50/50 dark:bg-rose-900/10 rounded-xl p-5 border border-rose-100 dark:border-rose-900/30">
                    ...
                    <ul className="space-y-2">
                        ...
                        {insights.riskFactors.length === 0 && (
                            <li className="text-sm text-muted-foreground italic">No significant risks identified.</li>
                        )}
                    </ul>
                </div>

                {/* 5. Expense Red Flags */}
                <div className="bg-slate-50 dark:bg-card/30 rounded-xl p-5 border border-slate-200 dark:border-slate-800">
                    <h3 className="text-sm font-semibold text-slate-900 dark:text-foreground mb-3 flex items-center gap-2">
            ...
            <div className="bg-white dark:bg-card rounded-xl p-5 border border-slate-200 dark:border-slate-800 shadow-sm">
                ...
                <div className="grid grid-cols-2 gap-4">
                    <div className="p-3 bg-slate-50 dark:bg-muted/50 rounded-lg">
                        <div className="text-xs text-muted-foreground/70 mb-1">Low Occupancy (-15%)</div>
                        <div className={cn(
                            "text-sm font-bold",
                    ...
                    <div className="p-3 bg-slate-50 dark:bg-muted/50 rounded-lg">
                        <div className="text-xs text-muted-foreground/70 mb-1">High Rates (+2%)</div>
                        <div className={cn(
                            "text-sm font-bold",
