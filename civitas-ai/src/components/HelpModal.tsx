...
export const HelpModal: React.FC<HelpModalProps> = ({ isOpen, onClose }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-popover border border-black/8 rounded-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden shadow-2xl flex flex-col">
                {/* Header */}
                <div className="flex items-center justify-between p-6 border-b border-black/8 bg-black/[0.02]">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-muted-foreground">Learn how to use Civitas</p>
                        </div>
                    </div>
                    <button
                        onClick={onClose}
                        className="p-2 rounded-lg text-muted-foreground/70 hover:text-foreground hover:bg-black/8 transition-all"
                    >
                        <X className="w-5 h-5" />
                    </button>
                </div>
                ...
                <div className="flex flex-1 overflow-hidden">
                    {/* Sidebar */}
                    <div className="w-64 border-r border-black/8 p-4 overflow-y-auto bg-black/[0.01] custom-scrollbar">
                        <nav className="space-y-1">
                            {Object.entries(topics).map(([key, topic]) => (
                                ...
                                >
                                    <span className={selectedTopic === key ? 'text-blue-400' : 'text-muted-foreground/70'}>
                                        {topicIcons[key]}
                                    </span>
                                    <span className={selectedTopic === key ? 'text-foreground font-semibold' : ''}>{topic.title}</span>
                                </button>
                            ))}
                        </nav>
                    </div>
                        ...
                        {loading ? (
                            <div className="flex items-center justify-center h-full">
                                <div className="relative">
                                    <div className="w-12 h-12 rounded-full border-2 border-black/8"></div>
                            ...
                            <div className="prose prose-sm max-w-none">
                                ...
                                <div className="whitespace-pre-line text-foreground/70 leading-relaxed">
                ...
                {/* Footer */}
                <div className="p-4 border-t border-black/8 bg-black/[0.02]">
